...
function buildUserPrompt(input: GenerateNotebookNoteDraftInput) {
  ...
  return [
    ...
    "2. content 必須比普通提醒更完整，約 180–420 字，可以分行。",
    "3. content 必須包含以下小標籤，並按此順序寫：",
    "   【評分準則】指出本筆記對應的準則。",
    "   【原文例子】引用或概括學生文章中一個短例子；如果沒有合適逐字引用，就用『這篇文章在……位置』具體描述。",
    "   【可記住的方法】把做法說成學生下次可直接執行的 1–2 步。",
    "   【示範改寫】提供一句可模仿的中文示範句；可以改寫原文局部，但不要代寫整段。",
    "   【下次檢查】給 2–3 個簡短檢查點。",
    "4. 示範句要貼近本文題材和學生程度，不要寫成通用金句。",
    "5. 如果準則是「內容」，重點放在扣題、材料、細節、情感或立意；如果是「表達」，重點放在句式、詞語、語氣、描寫或修辭；如果是「結構」，重點放在開首、段落、過渡、詳略或結尾；如果是「標點」，重點放在標點規則和例句。",
    "",
    "請輸出以下 JSON：",
    "{",
    '  "title": "不超過 20 字的筆記標題",',
    '  "content": "包含【評分準則】【原文例子】【可記住的方法】【示範改寫】【下次檢查】的完整筆記",',
